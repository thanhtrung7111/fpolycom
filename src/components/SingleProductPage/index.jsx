...
export default function SingleProductPage() {
  ...
  useEffect(() => {
    dispatch(getProductDetail({ maSanPham: id }));
    dispatch(loadEvaludate({ maSanPham: id }));
  }, [id, dispatch]);
          ...
          <div className="breadcrumb-wrapper w-full ">
            <div className="container-x mx-auto">
              <BreadcrumbCom
                paths={[
                  { name: "Trang chủ", path: "/" },
                  { name: "Sản phẩm", path: `/single-product/${id}` },
                ]}
              />
            </div>
          </div>
            ...
            <div className="container-x mx-auto">
              {tab === "des" && (
                <div data-aos="fade-up" className="w-full tab-content-item">
                  ...
                  <p className="text-[15px] text-qgray text-normal">
                    {productDetail.data?.moTa}
                  </p>
                  <p className="text-[15px] text-qgray text-normal mb-10">
                    {productDetail.data?.motaOption}
                  </p>
                  <div>
                    <h6 className="text-[18px] font-medium text-qblack mb-2">
                      Phân loại sản phẩm
                      ...
                      {productDetail.data?.sanPhamChiTietList?.map((item) => {
                        return (
                          <li className="font-normal text-qgray leading-9">
                            {productDetail.data?.tenSanPham} {item.tenSPCT}
                          </li>
                        );
                      })}
                    </ul>
              ...
              {tab === "info" && (
                <div data-aos="fade-up" className="w-full tab-content-item">
                  <SallerInfo data={productDetail.data?.cuaHang} />
                </div>
              )}
            </div>
          </div>
