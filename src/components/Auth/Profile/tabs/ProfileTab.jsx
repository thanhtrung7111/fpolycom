...
export default function ProfileTab() {
  ...
  const schema = new Yup.object().shape({
    ...
    gioiTinh: Yup.bool().required("Không để trống giới tính!"),
    soDienThoai: Yup.string().required("Không để trống điện thoại!"),
    tenNguoiDung: Yup.string().required("Tên người dùng không để trống!"),
  });

  useEffect(() => {
    if (profileUser?.data != null) {
      setInitialState(profileUser?.data);
    }
  }, [profileUser?.data]);

  const handleUpdate = async (values) => {
    // if (values.hinhAnh == null && profileImg == null) {
    //   console.log("hello");
    //   return;
    // }
    let body = null;
    if (profileImg != null) {
      ...
      form1.append("image", profileImg);
      const image = await api
        .post("/file/upload", form1)
        .then((response) => response.data)
        .catch((e) => {
          return;
        });
      values.hinhAnh = image;
      body = { ...values, maUser: currentUser.data?.maAccount };
    }

    body = { ...values, maUser: currentUser.data?.maAccount };
    dispatch(updateProfileUser({ ...body }));
    console.log(values);
  };
        ...
        {({ errors, touched, setFieldValue }) => (
          <Form>
                ...
                <div className="input-item flex space-x-2.5 mb-8">
                  <div className="w-1/2 h-full">
                    <div className="input-com w-full h-full">
                      ...
                      <div
                        className={`${
                          errors.diaChi && touched.diaChi && "border-red-500"
                        }  input-wrapper border border-qgray-border w-full h-full overflow-hidden relative`}
                      >
                        <Field
                          name="diaChi"
                    ...
                    <div className="flex items-center gap-x-5">
                      <div className="flex gap-x-2">
                        <Field
                          ...
                          name="gioiTinh"
                          value={true}
                          onChange={(e) => setFieldValue("gioiTinh", true)}
                          className={``}
                        />
                        Nam
                      </div>
                      <div className="flex gap-x-2">
                        <Field
                          ...
                          value={false}
                          name="gioiTinh"
                          onChange={(e) => setFieldValue("gioiTinh", false)}
                          className={``}
                        />
                        Nữ
                      </div>
            ...
            <div className="action-area flex space-x-4 items-center">
              <button
                disabled={profileUser.isLoading}
                type="submit"
                className={`${
                  profileUser.isLoading ? "disabled:opacity-75" : ""
                } w-[164px] h-[50px] bg-qblack text-white text-sm`}
              >
                {profileUser.isLoading ? <Loading /> : "Cập nhật"}
              </button>
            </div>
          </Form>
        )}
